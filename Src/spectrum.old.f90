...
!------  Use a default value to signal if it is known or not
  ...
  do while(.not. found .and. io_error == 0)
     ...
     if(ia-1 == iap .and. symb == symbb)then
        ...
        ilab,spectrum,spin,par,t12,ng,jf,part_decay_prob
        found = .true.
...
!------   gamma transitions.
  ...
  do i = 1, nol
     ...
     read(line,'(i3,1x,f10.6,1x,f5.1,i3,1x,(e10.2),i3,1x,a1,32x,f9.4)')                 &
          ilab, spectrum, spin, par, t12, ng, jf, part_decay_prob
     ...
     if(ilab > 1 .and. part_decay_prob > 0.0d0)ifail = .true.
