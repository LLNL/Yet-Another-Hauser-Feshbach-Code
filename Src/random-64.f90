...
real(kind=8) function random_64(iseed)
!
!*******************************************************************************
!
!  Discussion:
...
!
   ...
   integer(kind=int64), intent(inout) :: iseed
   integer(kind=int64), save :: A, M, Q, R
   integer(kind=int64), save :: kk
   real(kind=8), save :: AM
   integer(kind=int64), save :: i1 = -1_int64, i2 = -1_int64
!----------------------------------------------------------
   if(iseed <= 0_int64 .or. i2 < 0_int64)then              !   Initialize the seed
      M = 9223372036854775097_int64
      A = 2946716567_int64
      Q = 3130050626_int64
      R = 1671854155_int64
      AM = nearest(1.0d0,-1.0d0)/M
      i2 = ior(ieor(88888888899999999_int64,abs(iseed)),1_int64)
      i1 = ieor(77777777555555555_int64, abs(iseed))
      iseed = abs(iseed) + 1
   end if
