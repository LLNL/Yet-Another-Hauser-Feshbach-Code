...
!-------------------------------------------------------------------
   ...
   character(len=132) :: command(200)    !   maximum of 100 commands - shouldn't be more
   integer(kind=4) :: command_rank(200)  !   being a bit lazy not wanting to run twice to count
...
!------   Check the file 'Saved-Fission-Parameters.txt'
   ...
   do ii = 1, 2
      if(ii == 1)then
         inquire(file=data_path(1:len_path)//'my-saved-parameters/saved-parameters.txt',exist=file_exist)
         if(file_exist)then
            open(unit=153,file=data_path(1:len_path)//'my-saved-parameters/saved-parameters.txt',status='old')
         else
            cycle
         end if
      elseif(ii == 2)then
         inquire(file=data_path(1:len_path)//'saved-parameters/saved-parameters.txt',exist=file_exist)
         if(file_exist)then
            open(unit=153,file=data_path(1:len_path)//'saved-parameters/saved-parameters.txt',status='old')
         else
            exit
         end if
      end if
      ...
      do while(.not. finished)
         ...
         if(.not. reading)then
            ...
            if(num_commands > 200)then
               if(iproc == 0)then
                  write(6,*)'Too many commands in saved file > 200'
