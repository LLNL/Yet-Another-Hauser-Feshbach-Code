...
\title{Monte Carlo Hauser-Feshbach computer code system to model nuclear reactions: YAHFC V3.65  }
...
\end{enumerate}
In YAHFC, the decays are conducted via Monte Carlo and data is collected at each step; for example, the emission energy, angle, etc., for emitted particle in both the stationary Lab frame and the reaction center-of-momentum reference system. Options exist to print data for each decay step for later analysis by the user. After completing the calculation, the data is collected via the exit channels and printed into libraries that can be translated in GNDS format.
...
If only the cross section data is desired for a calculation, the option ``${\texttt{xs\_only y}}$'' is provided (default = ${\texttt{n}}$). With this option, no angles are sampled for any of the events, and the Lorentz transformations from the decaying system to the center-of-momentum and the stationary  laboratory reference frames are not performed, as emission spectra are not printed. This option decreases the CPU time to complete the calculation by approximately a factor of two.
...
Data are printed in these directories related to the cross section and the emission spectra and angular distributions if option "${\texttt{xs\_only n}}$" is selected. For channel directories that do not contain inelastic reactions, data are printed associated with the channel and decays that terminate to a given state in the residual nucleus. The terminus states are either the ground state or an isomer defined in the calculation, and are noted with the label "${\texttt{LXXX}}$", where ${\texttt{XXX}}$ is the state number (starting at zero for the ground state). For each channel, the following files appear for each terminus state:\\
\\
${\texttt{Channel\_label\_cs\_LXXX.dat}}$ \\
${\texttt{Channel\_label\_Eout\_Ang\_G\_Ein\_LXXX.dat}}$ \\
${\texttt{Channel\_label\_Eout\_Ang\_Leg\_G\_Ein\_LXXX.dat}}$\\
${\texttt{Channel\_label\_Eout\_G\_Ein\_LXXX.dat}}$\\
${\texttt{Channel\_label\_Ang\_G\_Ein\_LXXX.dat}}$\\
\\
where ${\texttt{label}}$ denotes the channel label, e.g., ${\texttt{2n}}$ for two emitted neutrons. The first file contains the cross section for each incident energy. In the second file, the emission spectra and angular distributions for each incident energy are printed as a probability-density function, $P(E,x|E_{in})$, with $x=\cos\theta$, and is normalized according to 
\begin{equation}
\int\int_{-1}^1 P(E,x|E_{in})dE dx = 1.
\end{equation}
In the file, the distributions are printed on equally spaced grids in $E$ and $x$ and are normalized over the distribution range using the trapazoidial rule.  The third file is similar to the second file except that the angular distribution is expanded with Legendre coefficients. In the fourth and fifth files, only the emission spectrum ($P(E|E_{in})$) and the angular distributions ($P(x|E_{in})$) are printed, respectively.  The total cross section for this channel is collected in the file ${\texttt{Channel\_label\_cs\_Tot.dat}}$. In the case where fission is present, the contribution to the fission cross section from this channel is printed in ${\texttt{Channel\_label\_cs\_fission.dat}}$

For inelastic channels, i.e., reactions with a single emitted particle that is identical to the projectile, two types of files are printed. The first represents reactions where the first, or primary decay, involves the emission of the projectile particle directly to a discrete state in the target nucleus. These data are collected in the files\\
\\
${\texttt{Inelastic\_cs\_LXXX.dat}}$ \\
${\texttt{Inelastic\_Ang\_LXXX.dat}}$ \\
${\texttt{Inelastic\_Ang\_leg\_LXXX.dat}}$ \\
\\
for the cross section, angular distribution of the emitted particle, and the Legendre coefficients of the angular distribution, respectively. As before, ${\texttt{LXXX}}$ denotes the discrete state in the target nucleus. These files contain direct reactions via coupled channels and DWBA as well as pre-equilibirum emission and decays for the compound nucleus. Note that these files begin with the label ${\texttt{L001}}$, as state 0 is the ground state, whose decays are collected in the files starting with name ${\texttt{Compound\_Elastic}}$. The second set of files, which are similar to the channel data files described above, represent all inelastic transitions that do not involve populating a discrete state in the target nucleus by the emission of the projectile particle directly to that state in the first decay of the cascade. These include a particle emitted to the continuous energy bins and rare cases where a photon is emitted prior to the emission of a particle. Similar to the channel data above, are the files\\
\\
${\texttt{Channel\_P\_cs\_LXXX.dat}}$ \\
${\texttt{Channel\_P\_Eout\_Ang\_G\_Ein\_LXXX.dat}}$ \\
${\texttt{Channel\_P\_Eout\_Ang\_Leg\_G\_Ein\_LXXX.dat}}$\\
${\texttt{Channel\_P\_Eout\_G\_Ein\_LXXX.dat}}$\\
${\texttt{Channel\_P\_Ang\_G\_Ein\_LXXX.dat}}$\\
\\
where ${\texttt{P}}$ denotes the particle label for for the projectile, and hence the label for inelastic channels. These files include all  reactions where the cascade of decays terminate in the ${\texttt{XXX}}$ level. In order to provide a more accurate description of the gamma transitions for these reactions, a second set of files\\
\\
${\texttt{Channel\_P\_cs\_entry\_to\_LXXX.dat}}$, \\
\\
containing the cross section for these decays at their entry point into the discrete spectrum in the target nucleus is provided. As a consequence, the gamma spectra in the channel files do not contain gamma transitions between discrete states in the cascade of decays. As noted above, these files contain transitions from the continuous energy bins in the target as well as the rare cases of photon decay in the composite nucleus followed by emission of the projectile particle. Consequently, rather weak cross section will be present in these files for incident energies below $E_{cut}$. It is to be noted that the sum of the channel files and the entry files are equal to the total cross sections appearing in  ${\texttt{Channel\_P\_cs\_Tot.dat}}$
...
The kinematics of the incident projectile, and the decay from each nucleus in the decay chain are computed using relativistic dynamics. Reactions are computed with an incident projectile, moving in the $z$-direction with kinetic energy $E_{Lab}$ impacting a target nucleus at rest in the laboratory frame. In the center-of-momentum frame (COM), the incident kinetic energy, $E_{in}$ (as used in Sec.~\ref{sec:Form}), of the projectile that is available to excite the compound nucleus is
\begin{equation}
E_{in} =  \sqrt{(M_p+M_T)^2 + 2M_T E_{Lab}}-(M_p+M_T),
\end{equation}
where $M_p$ and $M_T$ are the rest masses of the projectile and target nuclei (in units of energy), respectively. The center-of-momentum frame is then moving relative to the fixed laboratory frame in the positive $z$-direction with Lorentz transformation parameters
\begin{align}
\beta & = \frac{p_{Lab}}{M_T + M_p + E_{Lab}},\\
\gamma & = \frac{M_T+M_p + E_{Lab}}{(M_p+M_T)^2 + 2M_T E_{Lab}},
\end{align}
where $p_{Lab} = \sqrt{(M_p+E_{Lab})^2-M_p^2}$ is the momentum of the projectile in the laboratory frame. The Lorentz transformation from the COM frame back to the laboratory frame\footnote{Note the positive sign for the off-diagonal term $\gamma\beta$, this is due to the laboratory frame moving with velocity $\beta$ in the negative $z$-direction relative to the COM frame.} is
\begin{equation}
\Gamma_1 =
\begin{pmatrix}
...
\end{pmatrix}.
\end{equation}
By definition, the decay of the fist particle occurs in the COM frame.

For the decay of the $n^{th}$ particle with energy release $E_n$, the decay particle (d) and residual nucleus (R) exit back-to-back, with equal momenta of magnitude
\begin{equation}
p = \frac{\sqrt{E_E^4-M_d^2M_R^2}}{E_T},
\end{equation}
where $E_T=E_n+M_d+M_R$\footnote{Here, any excitation energy in the residual nucleus is included in the mass $M_R$.} is the total energy of the decaying nucleus and
\begin{equation}
E_E = \sqrt{\frac{E_T^2-M_d^2-M_R^2}{2}}.
\end{equation}
The cartesian components of the momentum of the decay particle are
\begin{align}
p_{d,1} & = p\cos(\theta) \\
p_{d,2} &= p\sin(\theta)\cos(\phi)\\
p_{d,3} & = p\sin(\theta)\sin(\phi).
\end{align}
The momentum of the residual nucleus is then $\vec p_R = - \vec p_d$, and the velocity of the recoil nucleus, which is the rest frame for the next decay, is 
\begin{equation}
\vec \beta = \frac{\vec p_{R}}{\gamma M_R},
\end{equation}
where  $\gamma = \sqrt{p^2+M_R^2}/M_R$. The Lorentz transformation back to the rest frame of the previous decay is
\begin{equation}
\Gamma_n =
\begin{pmatrix}
\gamma &  \gamma\beta_1 &  \gamma\beta_2  & \gamma\beta_3 \\
\gamma\beta_1  &  1+\gamma^\prime\beta_1^2 &  -\gamma^\prime\beta_1\beta_2  & -\gamma^\prime\beta_1\beta_3 \\
\gamma\beta_2 &- \gamma^\prime\beta_1\beta_2 & 1+\gamma^\prime\beta_2^2 & -\gamma^\prime\beta_2\beta_3 \\
\gamma\beta_3 & -\gamma^\prime\beta_1\beta_3 & -\gamma^\prime\beta_2\beta_3 & 1+\gamma^\prime\beta_3^2 \\
\end{pmatrix},
\end{equation}
where $\gamma^\prime = (\gamma-1)/\beta^2$. Thus, the energy and angles of the $n^{th}$ decay particle in the center-of-momentum and the laboratory frame are obtained via successive applications of Lorentz transformations to four-momentum $p_d$ of the decaying particle, i.e.,
\begin{align}
p_d^{COM} &= \Gamma_2 ...\Gamma_{n-1}p_d,\\
p_d^{Lab} &= \Gamma_1p_d^{COM}.
\end{align}
...
\hline
${\texttt{global\_fit\_gamma\_gamma}}$  &  ${\texttt{Answer}}$ &  Controls whether strength function is modified to fit to $\Gamma_\gamma$ \\
...
\hline
${\texttt{fit\_gamma\_gamma}}$  &  $Z$, $A$, ${\texttt{Answer}}$ &  Controls whether strength function is modified to fit to $\Gamma_\gamma$ \\
& & ${\texttt{Answer}}$ = ${\texttt{y/t/1}}$: Fit to $\Gamma_\gamma$ ({\bf Default}).\\
& & ${\texttt{Answer}}$ = ${\texttt{n/f/0}}$: Do not fit to $\Gamma_\gamma$ \\
& & Fit is achieved by adding a third $E1$ resonance centered at $E_0=5.0$~ MeV and width $\Gamma = 5.0$~MeV. The strength is fit to reproduce $\Gamma_\gamma$. Note that it is possible for this strength to be negative.\\
\hline
...
& & \hskip 1.1 cm j = 4: Soukhovitskii \& Capote    20 \\
